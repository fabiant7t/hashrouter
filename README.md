...
- `GET /{namespace}/{service}/by-addresses/{path...}` resolves endpoints, selects one via rendezvous hashing using `{path...}` as the key and each endpoint's sorted `addresses` CSV as candidates, and responds with `307` redirect to `http://{ip}:{port}/{path...}`
